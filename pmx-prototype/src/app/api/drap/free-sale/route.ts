...
export async function POST(request: NextRequest) {
  try {
    ...
    const { pdfBytes, sha256 } = await generateDRAPDocument('FREE_SALE', manufacturer, product, destination_country)

    const drapDir = path.join(process.cwd(), 'public', 'drap')
    await mkdir(drapDir, { recursive: true })
    const docRef = `FS-${manufacturer.drap_licence_no}-${Date.now().toString(36).toUpperCase()}`
    ...
    const result = await query(
      `INSERT INTO drap_documents (manufacturer_id, doc_type, product_id, destination_country, doc_ref, pdf_path, sha256_hash, generated_by, valid_until)
       VALUES ($1, 'FREE_SALE', $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
    ...
    console.error('Free sale cert generation error:', error)
    return NextResponse.json(
